/**
 * generate-gallery-pages — produce overview + 3 detail pages from
 * data/shared/gallery.yaml. Output:
 *   docs/gallery/intro.md (EN, root locale)
 *   docs/zh/gallery/intro.md (ZH)
 *   docs/gallery/{id}.md × N (EN)
 *   docs/zh/gallery/{id}.md × N (ZH)
 *
 * Usage: tsx scripts/build/generate-gallery-pages.ts
 */
import yaml from 'js-yaml'
import fs from 'node:fs'
import path from 'node:path'
import { GalleryTopicsFile } from './schema'

const SHARED = 'data/shared/gallery.yaml'
const OUT_EN_INTRO = 'docs/gallery/intro.md'
const OUT_ZH_INTRO = 'docs/zh/gallery/intro.md'
const OUT_EN_DIR = 'docs/gallery'
const OUT_ZH_DIR = 'docs/zh/gallery'
...
function mdExamplesTable(
  ...
  const header = isZh ? '| 作品 | 详情 | 备注 |' : '| Piece | Detail | Note |'
  const sep = '|---|---|---|'
...
try {
  ...
  ok++
  console.log(`  ✓ intro → gallery/{root,zh}/intro.md`)
} catch (e) {
...
for (const t of topics) {
  try {
    ...
    ok++
    console.log(`  ✓ ${t.id} → gallery/{root,zh}/${t.id}.md`)
  } catch (e) {
    console.error(`  ✗ ${t.id}: ${(e as Error).message}`)
  }
}

console.log(`\nGenerated ${ok}/${topics.length + 1} gallery pages`)
