---
title: Tourmaline
gem: tourmaline
---


# Tourmaline
<img src="../images/gems/tourmaline/tourmaline-gallery-1.jpg" alt="Tourmaline" style="max-width:100%;border-radius:var(--radius-md,4px);margin-bottom:1rem">





> Elbaite (tourmaline group)

<!-- language switcher hint -->
中文: [碧玺](/zh/gems/tourmaline)

---

## Classification

| Property | Value |
|---|---|
| Mineral Family | Elbaite (tourmaline group) |
| Formula | Na(Li,Al)₃Al₆(BO₃)₃Si₆O₁₈(OH)₃ |
| Crystal System | trigonal |

## Physical Properties

| Property | Value |
|---|---|
| Mohs Hardness | 7.25 |
| Specific Gravity | 3.06 |
| Refractive Index | 1.614-1.666 |

## Optical Properties

| Property | Value |
|---|---|
| Pleochroism | strong |
| Typical Colors | Pink, Green, Blue, Bi-color (watermelon), Black |
| Color Cause | Transition metals (Fe, Mn, Ti, Cu) produce diverse colors |

## Treatments & Disclosure

| Treatment | Details |
|-----------|---------|
| Common Methods | heat-treatment, irradiation |
| Disclosure Required | Yes |
| Note | Heat improves color; irradiation deepens light varieties |

## Gallery

<img src="../images/gems/tourmaline/tourmaline-gallery-2.jpg" alt="Tourmaline" style="max-width:32%;border-radius:var(--radius-md,4px);margin:0.25rem">
<img src="../images/gems/tourmaline/tourmaline-gallery-3.jpg" alt="Tourmaline" style="max-width:32%;border-radius:var(--radius-md,4px);margin:0.25rem">
<img src="../images/gems/tourmaline/tourmaline.jpg" alt="Tourmaline" style="max-width:32%;border-radius:var(--radius-md,4px);margin:0.25rem">
