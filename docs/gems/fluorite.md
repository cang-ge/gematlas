---
title: Fluorite
gem: fluorite
---


# Fluorite
<img src="../images/gems/fluorite/fluorite.jpg" alt="Fluorite" style="max-width:100%;border-radius:var(--radius-md,4px);margin-bottom:1rem">





> Halide (calcium fluoride)

<!-- language switcher hint -->
中文: [萤石](/zh/gems/fluorite)

---

## Classification

| Property | Value |
|---|---|
| Mineral Family | Halide (calcium fluoride) |
| Formula | CaF₂ |
| Crystal System | cubic |

## Physical Properties

| Property | Value |
|---|---|
| Mohs Hardness | 4 (Soft — carvings or collector facets) |
| Specific Gravity | 3.18 |
| Refractive Index | 1.43 |

## Optical Properties

| Property | Value |
|---|---|
| Pleochroism | weak |
| Typical Colors | Purple, Green, Yellow, Blue, Rainbow |
| Color Cause | Rare-earth and colour centres |

## Treatments & Disclosure

| Treatment | Details |
|-----------|---------|
| Common Methods | irradiation |
| Disclosure Required | No |
| Note | 'Rainbow' fluorite fluoresces; glass imitations common |

## Origin

| Region |
|---|
| China |
| Mexico |
| South Africa |
| Derbyshire, UK |

## History & Lore

Fluorite gave its name to fluorescence itself; its colour range rivals any mineral.


## Gallery

<img src="../images/gems/fluorite/fluorite-gallery-1.jpg" alt="Fluorite" style="max-width:32%;border-radius:var(--radius-md,4px);margin:0.25rem">
<img src="../images/gems/fluorite/fluorite-gallery-2.jpg" alt="Fluorite" style="max-width:32%;border-radius:var(--radius-md,4px);margin:0.25rem">
<img src="../images/gems/fluorite/fluorite-gallery-3.jpg" alt="Fluorite" style="max-width:32%;border-radius:var(--radius-md,4px);margin:0.25rem">
